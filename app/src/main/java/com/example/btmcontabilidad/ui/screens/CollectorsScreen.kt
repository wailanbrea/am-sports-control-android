...
fun CollectorsScreen(
    ...
    if (showCreateDialog) {
        CreateCollectorDialog(
            ...
            onConfirm = { name, email, pass ->
                viewModel.createCollector(name, email, pass)
            }
        )
    }
}
...
@Composable
private fun CollectorCard(collector: CollectorDto) {
    Card(
...
private fun CreateCollectorDialog(
    ...
    AlertDialog(
        ...
        title = {
            Text("Nuevo Cobrador", fontWeight = FontWeight.Bold)
        },
        text = {
            Column(
